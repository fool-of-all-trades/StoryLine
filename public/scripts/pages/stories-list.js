...
if (list) {
      ...
      items.forEach((item) => {
        ...
        if (item.is_anonymous) {
          ...
          authorHtml = `Author: <a href="/user/${
            item.user_public_id
          }">${window.escapeHtml(item.username ?? "user")}</a> · `;
        } else if (item.guest_name) {
          // logged-out user, no link to profile, but we have a nick he provided
          authorHtml = `Author: ${window.escapeHtml(item.guest_name)} · `;
        } else {
        ...
        li.innerHTML = `
          ...
          item.title ? window.escapeHtml(item.title) : "(no title)"
        }</a>
            <div class="meta">
              ${authorHtml}
          ...
          item.flower_count ?? 0
            ...
            <p class="preview">${window
              .escapeHtml(item.content)
              .slice(0, 180)}${item.content.length > 180 ? "…" : ""}</p>
  ...
  list.addEventListener("click", async (e) => {
    ...
    if (res.ok) {
      btn.parentElement.querySelector("[data-count]").textContent = data.count;
